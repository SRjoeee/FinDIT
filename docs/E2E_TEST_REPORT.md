# E2E 测试报告 — Stage 3.7 VisionField 重构 + Stage 3.6 管线修复
...
| 视频           | 大小          | Stage 3.5 耗时    | Stage 3.6 耗时    | Stage 3.7 耗时    | 3.7 vs 3.6 |
| ------------ | ----------- | --------------- | --------------- | --------------- | ---------- |
| 06290011.MP4 | 384M        | 100 秒*          | 57 秒*           | 39 秒*           | **-32%**   |
| 06290017.MP4 | 480M        | 53 秒            | 48 秒            | 46 秒            | **-4%**    |
| 06290008.MP4 | 544M        | 39 秒            | 49 秒            | 50 秒            | +2%        |
| 06290010.MP4 | 768M        | 52 秒            | 69 秒            | 63 秒            | **-9%**    |
| 06290001.MP4 | 864M        | 66 秒            | 83 秒            | 79 秒            | **-5%**    |
| **合计**       | **~3.1 GB** | **5:10 (310s)** | **5:06 (306s)** | **4:37 (277s)** | **-9%**    |


 视频 1 包含首次 WhisperKit 模型初始化 (~20-40s)

**Stage 3.7 说明**: VisionField 重构为纯代码结构变更（enum + 数据驱动遍历），不改变任何处理逻辑。-9% 的改善主要来自运行环境波动（WhisperKit 初始化 / Gemini API 响应时间 / 磁盘缓存）。**结论：零性能退化。**

**Stage 3.6 耗时增加原因**: 视频 3-5 耗时增加是因为 **Gemini Vision 现在分析了全部 17/17 个 clip**（之前只分析 6/17），每多分析一个 clip 需要额外的 API 调用时间 (~7-10s/clip)。这是质量大幅提升带来的合理开销。
...
### Stage 3.7 VisionField 重构验证

纯代码结构重构，无功能变更：

- **405 个单元测试**: 全部通过（+17 VisionField 新增测试）
- **E2E 管线**: 5 视频 17 场景，所有步骤正常
- **性能**: 277s vs 306s (-9%)，无退化
- **STT 引擎**: SpeechAnalyzer（WhisperKit 作后备，未使用）
- **重构效果**: 新增字段从 ~13 处改动减至 6 处
...
- 本地视觉分析 ✓
- Gemini Vision 分析 ✓
- STT 转录 ✓ (SpeechAnalyzer 优先)
- 向量嵌入 ✓
- 混合搜索 ✓
- VisionField 数据驱动 ✓ (可扩展)
